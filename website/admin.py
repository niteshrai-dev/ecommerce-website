...
def add_shop_items():
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            file_name = secure_filename(file.filename) # it will replace white space/invalid charater(i.e. #$..) in file name with underscore.
...
def update_item(item_id):
    if current_user.id ==1:
        ...
        if form.validate_on_submit():
            ...
            try:
                Product.query.filter_by(id=item_id).update(dict(product_name=product_name,current_price=current_price,previous_price=previous_price,in_stock=in_stock,flash_sale=flash_sale, product_picture=file_path))
                db.session.commit()
...
def update_order(order_id):
    if current_user.id == 1:
        ...
        if form.validate_on_submit():
            ...
            try:
                db.session.commit()
                flash(f'Order {order_id} Update Successfully')
